...
// // (async () => {
// //   // Launch Puppeteer with specific download preferences
...
// //           // Wait a bit to allow the download to start
...
//       if (stat.isDirectory()) {
//           results.directories.push(scanDirectory(fullPath));
//       } else {
...
// function logExtensions(results, indent = 0) {
//   const indentStr = ' '.repeat(indent);
  
//   // Extract just the last part of the path for display
//   const displayPath = path.basename(results.path);
  
//   console.log(`${indentStr}Category: ${displayPath}`);
  
//   if (results.files.length > 0) {
//       console.log(`${indentStr}  Files:`);
//       const extensions = {};
...
//           console.log(`${indentStr}    ${ext}: ${count} file(s)`);
//       }
//   }
  
//   results.directories.forEach(dir => {
//       logExtensions(dir, indent + 2);
//   });
// }

// // Usage
// const downloadFolderPath = "/home/victor/Desktop/work/ai_bid_agent/crawler/downloads"
// console.log("🚀 ~ downloadFolderPath:", downloadFolderPath)
// // Or specify an abso/lute path like: '/path/to/your/download/folder'

// try {
//   const scanResults = scanDirectory(downloadFolderPath);
//   logExtensions(scanResults);
// } catch (error) {
...
function getFileExtensions(directoryPath) {
    const results = {};
    ...
    for (const item of items) {
        console.log("🚀 ~ getFileExtensions ~ item:", item)
        const fullPath = path.join(directoryPath, item);
        const stat = fs.statSync(fullPath);
        
        if (stat.isDirectory()) {
            // If it's a directory, recurse into it
            results[item] = getFileExtensions(fullPath);
            console.log("🚀 ~ getFileExtensions ~ results:", results)
            
        } else {
            // If it's a file, get its extension
            const ext = path.extname(item).toLowerCase();
            console.log("🚀 ~ getFileExtensions ~ ext:", ext)
            return ext; // Return just the extension for files
        }
    }
...
// Usage: Replace with your actual download folder path
const downloadFolderPath = "/home/victor/Desktop/work/ai_bid_agent/crawler/downloads"

try {
    const extensionsByCategory = getFileExtensions(downloadFolderPath);
    console.log("🚀 ~ extensionsByCategory:", extensionsByCategory)
    console.log('File extensions by category:');
    console.log(JSON.stringify(extensionsByCategory, null, 2));
} catch (error) {
    console.error('Error scanning directory:', error);
}